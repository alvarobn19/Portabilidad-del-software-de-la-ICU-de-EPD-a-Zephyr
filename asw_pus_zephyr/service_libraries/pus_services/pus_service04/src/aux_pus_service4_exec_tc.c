...
LOG_MODULE_REGISTER(pus_service4_exec_tc);

#ifndef EXEC_START_ACK_ENABLED_MASK
#define EXEC_START_ACK_ENABLED_MASK 0x02
#endif
#ifndef EXEC_COMPLETION_ACK_ENABLED_MASK
...
void pus_service4_exec_TC_4_6(tc_handler_t *ptc_handler) {
        ...
        if (error) {
                ...
                // Svc3-like: liberar memoria siempre
                return;
        } else {
                ...
                if (1 != N) {
                        ...
                        if (sys_data_pool_is_PID_valid(PID)) {
                                ...
                                ptc_handler->tc_df_header.version_ackflags |=
                                                EXEC_START_ACK_ENABLED_MASK |
                                                EXEC_COMPLETION_ACK_ENABLED_MASK;
                               // printk("[S4/6] about to TM[1,3]\n");
                               int r1 = pus_service1_tx_TM_1_3(ptc_handler);
                               // printk("[S4/6] TM[1,3] ret=%d\n", r1);

                                //Try to add the stats of this param
                                error = pus_service4_add_PID_stats(PID);

                                if (!error) {
                                       // printk("[S4/6] about to TM[1,7]\n");
                                       int r7 = pus_service1_tx_TM_1_7(ptc_handler);
                                       // printk("[S4/6] TM[1,7] ret=%d\n", r7);

                                } else {

                                        // printk("[S4/6] sending TM[1,8] (err)\n");
...
void pus_service4_exec_TC_4_7(tc_handler_t *ptc_handler) {
        ...
        // printk("### HOTFIX: S4.7 delete stats (ENTER) ###\n");
                ...
                if (1 != N) {
                        ...
                        if (sys_data_pool_is_PID_valid(PID)) {
                                ...
                                ptc_handler->tc_df_header.version_ackflags |=
                                                EXEC_START_ACK_ENABLED_MASK |
                                                EXEC_COMPLETION_ACK_ENABLED_MASK;
                               // printk("[S4/7] about to TM[1,3]\n");
                               int r1 = pus_service1_tx_TM_1_3(ptc_handler);
                               // printk("[S4/7] TM[1,3] ret=%d\n", r1);

                                //Try to delete the stats of this param
                                error = pus_service4_delete_PID_stats(PID);

                                if (!error) {
                                       // printk("[S4/7] about to TM[1,7]\n");
                                       int r7 = pus_service1_tx_TM_1_7(ptc_handler);
                                       // printk("[S4/7] TM[1,7] ret=%d\n", r7);

                                } else {

                                        // printk("[S4/7] sending TM[1,8] (err)\n");
