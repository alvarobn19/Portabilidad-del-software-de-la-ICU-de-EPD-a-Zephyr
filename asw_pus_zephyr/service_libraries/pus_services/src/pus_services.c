...
extern void cdevaction_exec_next_from_queue(void);
		...
		if (pus_service12_is_PMON_enabled(i)) {
				...
				if (event_triggered) {
                                        ...
                                        if (event_enabled) {
                                                ...
                                                pus_service1_tx_TM_5_X_param_out_of_limit(EvID,
                                                                &fault_info);
                                                fault_pid = fault_info.PID;
                                                fault_value = fault_info.PIDValue.uint8_data;
			...
			//If event triggered && event enabled mng event action
                        ...
                        if(event_triggered && event_enabled){
                                printk("[FDIR] Event 0x%04X triggered by PID=%u value=%u (>HL)\n",
                                                EvID, fault_pid,
                                                fault_value); // DIAG

                                pus_service19_mng_event_action(EvID);
                                cdevaction_exec_next_from_queue();
                        }
                }
        }
}
