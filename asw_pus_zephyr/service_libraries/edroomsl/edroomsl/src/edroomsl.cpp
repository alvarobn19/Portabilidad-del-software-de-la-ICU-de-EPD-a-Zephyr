...
CEDROOMTimingSAP::CEDROOMTimingSAP(CEDROOMComponent *p_Component_,
                ...
                m_SynchrSem((unsigned int) 0), m_TimerTask(),
...
void CEDROOMTimingSAP::Start() {
        const TEDROOMPriority prio = mp_Component->GetNomPriority();
        const unsigned stack = _EDROOM_TIMING_TASK_STACK;

        new (&m_TimerTask) Pr_Task(&CEDROOMTimingSAP::EDROOMfunTimer, prio, stack);

        mp_TimerTask = &m_TimerTask;

        Pr_Send(m_TimerTask, this);
}
...

{
	...
	for (unsigned int i = 0; i < m_PrioNumber; i++)
        ...
        //printk("[EDR][queue %p] new m_NewMsgSem=%p\n", this, &m_NewMsgSem);
...
void CEDROOMQueue::In(CEDROOMMessage *pMsg) {
	...
	// Wake Receptor.
        ...
        if (m_IsBlocked) {
                ...
                m_IsBlocked = 0;
                //printk("[EDR][%p] In: queue=%p sem=%p give\n", mp_Task, this, &m_NewMsgSem);
                m_NewMsgSem.Signal();
...
TEDROOMBool CEDROOMQueue::Out(CEDROOMMessage *&p_Msg) {
	...
	mp_MutexSem->Wait();
       ...
       if (!mp_FirstQueueHead) {
               ...
               //printk("[EDR][%p] queue=%p sem=%p -> blocking\n", mp_Task, this, &m_NewMsgSem);
	...
	m_MsgCounter--;
       ...
       p_Msg = p_node->mp_Msg;
       //printk("[EDR][%p] dequeued signal=%u prio=%u\n", mp_Task,
              //(unsigned)p_Msg->signal, (unsigned)p_Msg->priority);
...

{
	...
	//printk("[EDR] boot-send: component=%p -> task=%p\n", this, mp_ComponentTask);
	Pr_Send(*mp_ComponentTask, this);
...
Pr_TaskRV_t CEDROOMComponent::EDROOMComponentTask(Pr_TaskP_t) {
        CEDROOMComponent *ap_component = nullptr;
        Pr_Receive(&ap_component, sizeof(ap_component));
        //printk("[EDR] boot-recv: ap_component=%p\n", ap_component);
        if (!ap_component) {
                // printk("[EDR] ERROR: ap_component NULL; no se recibió el puntero\n");
                return;
        }
