...

{
  ...
  CDTCHandler & varSBKGTC = *(CDTCHandler *)Msg->data;
                ...
                // Data access
   tc_handler_t *th = varSBKGTC.GetTCHandler();
   ...
   printk("[BKGTCExec] Ignoring ExecTC for ReqID(%u,%u,%u,%u) handler=%p\n",
          apid, th->tc_df_header.type, th->tc_df_header.subtype, seq, th);

   // No execution here; background executor will handle it
}
