...
// Tamaño de pila para las tareas de bottom-half de IRQ
#ifndef EDROOM_IRQ_BH_STACK
#define EDROOM_IRQ_BH_STACK  4096*4   // o 8192 si prefieres ir sobrado
#endif
...
Pr_IRQHandler_RetType   CCTCManager::EDROOMIRQ18Handler(const void *){
...
Pr_IRQHandler_RetType   CCTCManager::EDROOMIRQ18IdleHandler(const void *){
	return;
}
...
Pr_TaskRV_t 	CCTCManager::EDROOMIRQ18BottomHalfTask(Pr_TaskP_t){
	//printk("[BH] IRQ18 bottom-half arrancado; esperando evento...\n");
	bool endTask=false;
	//printk("endTask=false\n");

        do
        {

                EDROOMEventIRQ18.Wait();
                //printk("[BH] señal recibida\n");

                bool EDROOMIRQ18SendMsgToCmp=true;
                //printk("do\n");
		if(!EDROOMSemEndIRQ18.WaitCond()){

			//printk("Dentro del if\n");
	...
	//printk("[BH] IRQ18 bottom-half finalizado...\n");
...
CCTCManager::CCTCManager(TEDROOMComponentID id,
		...
		EDROOMIRQ18BottomHalfT( EDROOMIRQ18BottomHalfTask,EDROOMprioURGENT,EDROOM_IRQ_BH_STACK),
